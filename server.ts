...
function shouldAllowAnyOrigin() {
  return process.env.NODE_ENV !== 'production' && getAllowedOrigins().length === 0
}
...
const fieldMapping: Record<string, any> = {
  ...
  refusalMarketingAuthorisationDate: medicines.refusalMarketingAuthorisationDate,
  withdrawalExpiryRevocationDate: medicines.withdrawalExpiryRevocationDate,
  suspensionMarketingAuthorisationDate: medicines.suspensionMarketingAuthorisationDate,
...
const buildWhereClause = (queryParams: Record<string, any>) =>
  Effect.sync(() => {
    ...
    if (queryParams.lastUpdatedDateFrom) {
      conditions.push(sql`${medicines.lastUpdatedDate} >= ${queryParams.lastUpdatedDateFrom}`)
    }
    if (queryParams.lastUpdatedDateTo) {
      conditions.push(sql`${medicines.lastUpdatedDate} <= ${queryParams.lastUpdatedDateTo}`)
    }
...
const getTotalCount = (db: BunSQLiteDatabase, whereClause: any) =>
  Effect.tryPromise({
    try: async () => {
      const res = await db.select({ total: count() }).from(medicines).where(whereClause)
      return res[0]?.total
    },
...
const getPaginatedData = (
  ...
  Effect.tryPromise({
    try: () => {
      ...
      return db.select().from(medicines).where(whereClause).limit(pageSize).offset(offset)
    },
    catch: (error) => new DatabaseError(error),
  })
...
export function createMedicineAPI(db: BunSQLiteDatabase) {
  ...
  const allowAnyOrigin = shouldAllowAnyOrigin()
      ...
      swagger({
        ...
        documentation: {
          info: {
            title: 'Medicines API',
            version: '1.0.0',
            description: 'API for searching and retrieving medicine information from EMA database',
          },
          tags: [
    ...
    .get(
      ...
      async ({ query }) => {
        const effect = Effect.gen(function* (_) {
          ...
          const data = yield* _(getPaginatedData(db, whereClause, page, pageSize))
        ...
        detail: {
          ...
          parameters: [
            ...
            },
            {
              ...
              in: 'query',
              description: 'Filter by marketing authorisation date from (inclusive)',
              required: false,
            ...
            },
            {
              ...
              in: 'query',
              description: 'Filter by marketing authorisation date to (inclusive)',
              required: false,
              schema: { type: 'string', format: 'date' },
            },
            {
    ...
    .get(
      ...
      async ({ params }) => {
        const effect = Effect.gen(function* (_) {
          ...
          if (!field) {
            return yield* _(Effect.fail(new HTTPError(400, 'Invalid field name')))
          }
    ...
    .get(
      ...
      async ({ params }) => {
        const effect = Effect.gen(function* (_) {
          ...
          if (results.length === 0) {
            return yield* _(Effect.fail(new HTTPError(404, 'Medicine not found')))
          }
...
Available endpoints:
  ...
  if (allowAnyOrigin) {
    console.warn(
      'CORS: allowing any origin because NODE_ENV is not production and CORS_ALLOWED_ORIGINS is unset',
    )
  } else if (allowedOrigins.length > 0) {
    console.log(`CORS: allowing ${allowedOrigins.join(', ')}`)
  } else {
    console.warn(
      'CORS: no allowed origins configured; cross-origin browser requests will be blocked',
    )
  }
