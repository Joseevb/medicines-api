...
const csvToTableMapping: Record<string, string> = {
  ...
  marketing_authorisation_developer__applicant__holder: 'marketingAuthorisationDeveloper',
  ...
  suspension_of_marketing_authorisation_date: 'suspensionMarketingAuthorisationDate',
...
const cleanHeader = (raw: string): string =>
  raw.trim().replace(/\n/g, ' ').replace(/ +/g, '_').replace(/[()/]/g, '').toLowerCase()
...
const insertRow = (db: BunSQLiteDatabase, validated: any, originalRow: Record<string, string>) =>
  Effect.tryPromise({
...
const processAllRows = (db: BunSQLiteDatabase, records: Array<Record<string, string>>) =>
  Effect.gen(function* (_) {
  ...
  Effect.gen(function* (_) {
    ...
    if (result.errors.length > 0) {
      ...
      if (result.errors.length > 5) {
        yield* _(Console.log(`... and ${result.errors.length - 5} more errors`))
      }
    }
